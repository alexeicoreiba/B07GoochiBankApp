...
public class DatabaseInsertHelper extends DatabaseInserter {
  ...
  public static int insertAccount(String name, BigDecimal balance, int typeId)
    ...
    int accountId = 0;
    try {
      accountId = DatabaseInserter.insertAccount(name, newBalance, typeId, connection);
      connection.close();
  ...
  public static int insertAccountType(String name, BigDecimal interestRate)
    ...
    int success = 0;
    try {
      success = DatabaseInserter.insertAccountType(name, interestRate, connection);
      connection.close();
    } catch (SQLException e) {
      e.printStackTrace();
    } catch (DatabaseInsertException e) {
  ...
  public static int insertNewUser(String name, int age, String address, int roleId, String password)
      throws InvalidNameException {
    Connection connection = DatabaseDriverHelper.connectOrCreateDataBase();
    // check if the name entered is not null
    if (name.equals(null)) {
      throw new InvalidNameException();
    }
   ...
   * @return true if successful, false otherwise.
   */
  public static int insertRole(String role) throws InvalidNameException {
    Connection connection = DatabaseDriverHelper.connectOrCreateDataBase();
    // check if name is null
    if (role.equals(null)) {
      throw new InvalidNameException();
    ...
    int success = 0;
    try {
      success = DatabaseInserter.insertRole(role, connection);
      connection.close();
    } catch (SQLException e) {
      e.printStackTrace();
    } catch (DatabaseInsertException e) {
  ...
  public static int insertUserAccount(int userId, int accountId) {
    ...
    if (userId > 0 && accountId > 0) {
      ...
      try {
        retVal = DatabaseInserter.insertUserAccount(userId, accountId, connection);
        connection.close();
      } catch (SQLException e) {
        e.printStackTrace();
      } catch (DatabaseInsertException e) {
